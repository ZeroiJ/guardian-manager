...
| **Loadout Application Pipeline**    | `src/app/loadout-drawer/loadout-apply.ts` | DIM-grade sequencing + notifications. **Partially done** — `equipManager.ts` transfers items, batches `EquipItems`, applies **socket overrides** and **armor mods** via `InsertSocketPlugFree`-style proxy calls; exotic conflicts surfaced via failure bitmask but not auto-resolved like DIM.                          | High       |
...
| Feature                                 | DIM Source Reference       | Description                                                                                                                                                                                                                                                                             | Effort     |
| --------------------------------------- | -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| ~~**Loadout Optimizer / Armor Builder**~~ | `src/app/loadout-builder/` | **DONE** (v0.35–v0.36) — Web Worker engine, stat constraints, exotic picker, auto stat mods, DIM-style results UI (`LoadoutOptimizer` page). | ~~Very High~~ |
...
| ~~**Socket Stripping**~~   | `src/app/strip-sockets/`             | **DONE** — `StripSockets.tsx` (+ worker/API) for stripping armor mods / optional shaders.                                                                                                                                      | ~~Low~~ |
| **Fashion System**         | `src/app/loadout/fashion/`           | **Partial** — Captured in loadout `socketOverrides`; apply path uses same plug-insert pipeline as subclass/fashion in `equipManager.ts`. Full DIM parity (ordering, energy) still incremental.                                  | Medium  |
...
| ~~**Item Feed**~~         | `src/app/item-feed/`                 | **DONE** — `itemFeedStore.ts` + `ItemFeedPanel` / nav button; snapshot diff on profile refresh.                                                                                     | ~~Low~~  |
| **Hashtag Notes**         | `src/app/inventory/note-hashtags.ts` | **Partial** — `note-hashtags.ts`, parsing/display in `ItemDetailOverlay`; full DIM-style note UX across all surfaces may still vary.                                                | Low      |
| ~~**Clarity Integration**~~ | `src/app/clarity/`               | **DONE** — `clarityStore.ts` loads Live Clarity DB; `ClarityInsight` in item UI.                                                                                                  | ~~Medium~~ |
| ~~**CSV Export**~~        | `src/app/inventory/spreadsheets.ts`  | **DONE** — Organizer page CSV export (`lib/organizer/utils`).                                                                                                                      | ~~Low~~  |
...
| **Compare**             | Multi-item `CompareModal` with session filter (`compareSession`).                                                                                                                                 | Optional upgrades: stronger “similar roll” suggestions, parity with every DIM compare nuance.                                         |
...
| **Notifications**       | Toast stack + `progress` notification type (`notificationStore` / `NotificationToast.tsx`).                                                                                                  | DIM-level bulk-move progress + undo stack not fully matched.                                                                          |
...
1. Harden the loadout application pipeline vs DIM *(transfers + equip + socket plugs + mods exist in `equipManager.ts`; continue improving exotic ordering, mod assignment sophistication, and progress UX.)*
...
1. ~~**Loadout Optimizer**: Tackle the Web Worker-based armor builder.~~ **Phase 1 DONE** (v0.35.0) — Core worker, types, pre-filter, stat constraints, results page. **Phase 2 DONE** (v0.36.0) — `addStats` bug fix, flat stat arrays, exotic picker, auto stat mods, DIM-style inline results with Bungie armor icons, equip button wired.
