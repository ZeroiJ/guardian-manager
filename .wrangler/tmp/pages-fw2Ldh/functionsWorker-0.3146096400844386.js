...
// config.ts
var getBungieConfig = /* @__PURE__ */ __name((env) => {
  return {
...
// manifest.ts
var getManifestMetadata = /* @__PURE__ */ __name(async () => {
...
var getManifestTablePath = /* @__PURE__ */ __name(async (table, language = "en") => {
  const manifest = await getManifestMetadata();
  const paths = manifest.jsonWorldComponentContentPaths[language];
  return paths ? paths[table] : void 0;
}, "getManifestTablePath");

// api/[[route]].ts
var app = new Hono2();
var CLIENT_ID = "51042";
var CLIENT_SECRET = "g4K4fPwbN0H-zewTWvAQPulHdB.yj7lx-UtJO6ZdIfE";
app.use("*", async (c, next) => {
...
app.get("/api/profile", async (c) => {
  ...
  const titan = Object.values(characters).find((c2) => c2.classType === 0);
  if (titan) {
    console.log("[Backend Debug] Titan Found:", titan.characterId);
    console.log("[Backend Debug] Titan Emblem:", `https://www.bungie.net${titan.emblemPath}`);
  }
...
app.get("/api/metadata", async (c) => {
  ...
  return c.json({
    ...
    notes: JSON.parse(metadata.notes || "{}")
  });
});
app.post("/api/metadata", async (c) => {
  ...
  const tokens = JSON.parse(authCookie);
  const membershipId = tokens.membership_id;
  const { itemId, type, value } = await c.req.json();
  const existing = await c.env.guardian_db.prepare(
    "SELECT tags, notes FROM UserMetadata WHERE bungieMembershipId = ?"
  ).bind(membershipId).first();
  let tags = JSON.parse(existing?.tags || "{}");
  let notes = JSON.parse(existing?.notes || "{}");
  if (type === "tag") {
...
// ../.wrangler/tmp/pages-fw2Ldh/functionsRoutes-0.4167272201433162.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
